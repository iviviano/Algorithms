...
- It looks like you were trying to handle multiple tree layers at once. For example, in `all-sum`, you fold over the children nodes of the current tree `t`. Instead of adding up the sums of the subtree rooted at each node, you look explicitly at its children. A simple correct solution for this case is `(map `

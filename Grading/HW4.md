...
Implementation Notes:
- Your `empty?` cases in `replace` and `weigh` are unnecessary, since `fold` over an empty list returns the base case. The `leaf?` case of `all-sum` is unnecessary for the same reason.

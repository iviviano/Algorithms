...
For each $t\in \Sigma$, add a transition $t,t \rightarrow \varepsilon$ from $q_\text{loop}$ to $q_\text{loop}$. For each rule $A \rightarrow u_{1}\cdots u_{n}$, add $n-1$ new states and transitions to pop $A$ push $u_{n},\ldots,u_{1}$
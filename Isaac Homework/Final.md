...
Proof of recurrence:
For the substring $s_{l(i)+1}\cdots s_{l(j)}$, we can first split at any index $l(i+1),\ldots,l(j-1)$. Suppose we split at $l(i+k)$ for some $1\le k\lt j-i$. We then get the two substrings $s_{l(i)+1}\cdots s_{l(i+k)}$ and $s_{l(i+k)+1}\cdots s_{l(j)}$. The minimum cost of finishing splitting the first substring is $\texttt{OPT}(i,i+k)$, and $\texttt{OPT}(i+k,j)$ for the second. So, the total cost of splitting 

The cost of splitting the substring is $l(j)-l(i)-1$. 
